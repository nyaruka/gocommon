...
// Writer provides buffered writes to OpenSearch using a batcher. If writes fail, they are added to the given
...
func NewWriter(client *opensearchapi.Client, maxItems int, maxAge time.Duration, bufferSize int, spool *Spool) *Writer {
	w := &Writer{
		client: client,
		spool:  spool,
	}
