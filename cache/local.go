...
// NewLocal creates a new in-memory cache. A positive capacity bounds it to that many items, evicting the
...
// below the number of keys in real use turns every eviction into a refetch. Zero, or anything less, leaves
// the cache unbounded.
func NewLocal[K comparable, V any](fetch Fetcher[K, V], ttl time.Duration, capacity int) *Local[K, V] {
	return &Local[K, V]{
		cache: ttlcache.New[K, V](
			...
			// clamped because a negative would wrap to a capacity so large it bounds nothing, which is the
			// opposite of what asking for one means
			ttlcache.WithCapacity[K, V](uint64(max(0, capacity))),
		),
		fetch: fetch,
	}
}
