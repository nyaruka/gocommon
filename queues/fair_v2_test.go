...
func TestFairV2(t *testing.T) {
	...
	task1UUID := assertPushV2(t, q, vc, "owner1", false, []byte(`task1`))
	task2UUID := assertPushV2(t, q, vc, "owner1", true, []byte(`task2`))
	task3UUID := assertPushV2(t, q, vc, "owner2", false, []byte(`task3`))
	task4UUID := assertPushV2(t, q, vc, "owner1", false, []byte(`task4`))
	task5UUID := assertPushV2(t, q, vc, "owner2", true, []byte(`task5`))
	...
	assertPopV2(t, q, vc, task2UUID, "owner1", "task2") // because it's highest priority for owner 1
	assertQueued(map[queues.OwnerID]int{"owner1": 2, "owner2": 2})
	assertActive(map[queues.OwnerID]int{"owner1": 1})

	assertPopV2(t, q, vc, task5UUID, "owner2", "task5") // because it's highest priority for owner 2
	assertQueued(map[queues.OwnerID]int{"owner1": 2, "owner2": 1})
	assertActive(map[queues.OwnerID]int{"owner1": 1, "owner2": 1})

	assertPopV2(t, q, vc, task1UUID, "owner1", "task1")
	...
	assertPopV2(t, q, vc, task4UUID, "owner1", "task4")
	assertPopV2(t, q, vc, task3UUID, "owner2", "task3")
	assertTasks("owner1", []string{}, []string{})
	assertTasks("owner2", []string{}, []string{})
	...
	assertPopV2(t, q, vc, "", "", "") // no more tasks
	assertTasks("owner1", []string{}, []string{})
	assertTasks("owner2", []string{}, []string{})
	...
	task6UUID := assertPushV2(t, q, vc, "owner1", false, []byte(`task6`))
	task7UUID := assertPushV2(t, q, vc, "owner1", false, []byte(`task7`))
	task8UUID := assertPushV2(t, q, vc, "owner2", false, []byte(`task8`))
	task9UUID := assertPushV2(t, q, vc, "owner2", false, []byte(`task9`))

	assertPopV2(t, q, vc, task6UUID, "owner1", "task6")
	...
	assertPopV2(t, q, vc, task8UUID, "owner2", "task8")
	assertPopV2(t, q, vc, task9UUID, "owner2", "task9")
	assertPopV2(t, q, vc, "", "", "") // no more tasks
	...
	assertPopV2(t, q, vc, task7UUID, "owner1", "task7")
	...
	assertPushV2(t, q, vc, "owner1", false, []byte("task10"))
	task11UUID := assertPushV2(t, q, vc, "owner2", false, []byte("task11"))
	...
	assertPopV2(t, q, vc, task11UUID, "owner2", "task11")
	assertPopV2(t, q, vc, "", "", "")
...
func TestFairV2TaskPayloads(t *testing.T) {
	...
	task1UUID := assertPushV2(t, q, vc, "owner1", true, []byte(`{"foo": "|"}`))
	task2UUID := assertPushV2(t, q, vc, "owner1", true, []byte(`task2`))

	assertPopV2(t, q, vc, task1UUID, "owner1", `{"foo": "|"}`)
	assertPopV2(t, q, vc, task2UUID, "owner1", "task2")
}
...
func TestFairV2MaxActivePerOwner(t *testing.T) {
	...
	task1UUID := assertPushV2(t, q, vc, "owner1", false, []byte(`task1`))
	task2UUID := assertPushV2(t, q, vc, "owner1", true, []byte(`task2`))
	task3UUID := assertPushV2(t, q, vc, "owner1", false, []byte(`task3`))

	assertPopV2(t, q, vc, task2UUID, "owner1", "task2")
	assertPopV2(t, q, vc, task1UUID, "owner1", "task1")
	assertPopV2(t, q, vc, "", "", "") // owner1 has reached max active tasks

	q.Done(ctx, vc, "owner1")

	assertPopV2(t, q, vc, task3UUID, "owner1", "task3") // now we can pop task3
}
...
// assertPushV2 is a helper function that asserts the result of a Push operation
func assertPushV2(t *testing.T, q *queues.FairV2, vc valkey.Conn, owner queues.OwnerID, priority bool, task []byte) queues.TaskID {
	ctx := t.Context()
...
// assertPopV2 is a helper function that asserts the result of a Pop operation
func assertPopV2(t *testing.T, q *queues.FairV2, vc valkey.Conn, expectedID queues.TaskID, expectedOwner queues.OwnerID, expectedTask string) {
	ctx := t.Context()
