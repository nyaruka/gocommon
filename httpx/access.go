...
// WithAccessControl wraps an http.RoundTripper so that it enforces the given access config before delegating to the
// inner transport. A nil access config makes it a pass-through, so it's always safe to wrap. If inner is nil then
// http.DefaultTransport is used.
func WithAccessControl(inner http.RoundTripper, access *AccessConfig) http.RoundTripper {
	if inner == nil {
